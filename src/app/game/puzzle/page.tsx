...
export default function PuzzlePage() {
  const regions = getAllRegions().map((region) => ({
    ...region,
    puzzleCount: getPuzzleItemsByRegion(region.slug).length,
  }));

  const totalPuzzles = regions.reduce((total, region) => total + region.puzzleCount, 0);
              ...
              >
                ...
                <div className="px-6 py-6">
                  <div className="flex items-start justify-between gap-4">
                    ...
                    <span className="rounded-full bg-[#f4ead4] px-4 py-2 text-sm font-bold text-[#8a5923]">
                      {region.puzzleCount} puzzle
                    </span>
                  </div>
                  <p className="mt-5 line-clamp-2 text-sm leading-6 text-[#625850]">
                    {region.summary}
                  </p>
                  <span className="mt-6 inline-flex h-11 w-full items-center justify-center rounded-[10px] bg-[#8f5a23] text-sm font-bold text-white transition group-hover:bg-primary">
                    Mulai Puzzle
                  </span>
                </div>
              </Link>
